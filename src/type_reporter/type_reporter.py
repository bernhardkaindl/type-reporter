...
import selectors
import shlex
from logging import DEBUG, INFO, basicConfig, debug, fatal, info
from os import environ as env, getcwd
from os.path import basename
from subprocess import PIPE, CalledProcessError, Popen, check_output  # nosec:B404
from sys import argv, exit as sys_exit, stderr, stdout
from typing import Any, List, TextIO, Tuple, TypedDict, cast
...
class Config(TypedDict):
    ...
    msg: str
    discard_messages_matching: list[str]
    xfail_section_description: str
    expected_to_fail: list[str]
    default_branch: str
...
def populate_error_dict(config, message, filename, lineno, code):
    # type: (Config, str, str, str, str) -> dict[str, str]
...
def github_error(config: Config, line: str) -> Tuple[str, InfoItem, str]:
    ...
    for ignore_string in config.get("discard_messages_matching", []):
        if re.match(ignore_string, message):
            return "", {}, ""

    error_dict = populate_error_dict(config, message, filename, lineno, code)
    error_dict["Function"] = f"`{func}`" if func else "syntax<br>error"
    ...
    title = f"pytype: {code}"
    return (
        # Generate a {typ} annotation for GitHub Actions to show in the PR review:
        f"::{typ} file={filename},line={lineno},title={title}::{message}",
        cast(InfoItem, error_dict),
        filename,
    )
...
def handle_grouping(filename: str, last_filename: str, log: TextIO) -> str:
    ...
    while ok:
        for key, _ in default_selector.select():
            ...
            if (
                ...
                info(line)
            if error_dict:
                if line == "":  # type: ignore[unreachable]
                    continue
...
def notice(config: Config, filename: str, title: str, message: str) -> None:
...
def annotate(kind: str, config: Config, filename: str, title: str, msg: str) -> None:
    ...
    print(f"::{kind} file={filename},title={script}: {title}::{msg}")
...
def gen_markdown(config: Config, out: TextIO, returncode: int, results: Info) -> None:
...
def write_summary_file(summary: str, anchor_text: str) -> None:
...
def run_pytype_and_generate_summary(config: Config) -> int:
    ...
    if summary_file:
        with open(summary_file, "w", encoding="utf-8") as fp:
            gen_markdown(config, fp, *result_tuple)
    else:
        gen_markdown(config, stdout, *result_tuple)
...
def load_config(config_file: str, script_basename: str) -> Config:
    ...
    config = cast(Config, pyproject["tool"].get(script_name) or {})
    ...
    try:
        ...
        try:
            repository_url = pyproject["project.poetry"]["repository"].strip(" /")
        except KeyError:
...
def find_branch_point(config: Config) -> str:
    ...
    try:
        return check_output(cmd, text=True).strip()  # nosec:B603
    except CalledProcessError as e:
        print(f"Error: {e}")
        return ""


def github_get_pr_commit_messages(repo: str, pr_number: str) -> list[str]:
...
def only_reverts_from_branch_point(config: Config, changed_files: List[str]) -> bool:
    ...
    if not msgs:
        ...
        out = check_output(cmd, text=True)  # nosec:B607 B603
        msgs = out.splitlines()
    ...
    old_ref = check_output(old_cmd, text=True).strip()
...
def main() -> int:
    ...
    merge_base = find_branch_point(config)
    if not merge_base:
        return run_pytype_and_generate_summary(config)

    config["changed_files"] = git_diff("--name-only", merge_base).splitlines()

    changed_but_in_expected_to_fail = [
        pull_request_file
        for pull_request_file in config["changed_files"]
        if pull_request_file in config["expected_to_fail"]
    ]

    if only_reverts_from_branch_point(config, changed_but_in_expected_to_fail):
        return run_pytype_and_generate_summary(config)
    ...
    for changed_xfail in changed_but_in_expected_to_fail:
        annotate(
            ...
            msg=f"{changed_xfail} was changed, remove it from expected_to_fail"
